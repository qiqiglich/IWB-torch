...
def print_accuracy(model, dataloaders, new_labelList, img_num_per_cls=None, device = 'cpu', test_aug = True):
    model.eval()
    ...
    predList = np.array([])
    grndList = np.array([])
    labels1 = [i for i in range(len(img_num_per_cls)) if img_num_per_cls[i]<20]
    ...
    recall = sklearn.metrics.recall_score(grndList, predList, labels=labels1, average='macro')
    ...
    breakdownResults = shot_acc(predList, grndList, np.array(new_labelList), many_shot_thr=100, low_shot_thr=20, acc_per_cls=True)
...
def horizontal_flip_aug(model):
    def aug_model(data):
        ...
        h_logits, features = model(data.flip(3))
