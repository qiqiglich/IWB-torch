...
def CB_loss(labels, logits, samples_per_cls, no_of_classes, loss_type, beta, gamma, device):
    ...
    if loss_type == "focal":
        ...
        cb_loss = F.binary_cross_entropy_with_logits(input=logits, target=labels_one_hot, weight=weights, reduction="sum")
        cb_loss /= torch.sum(labels_one_hot)
    elif loss_type == "softmax":
        pred = logits.softmax(dim=1)
        cb_loss = F.binary_cross_entropy(input=pred, target=labels_one_hot, weight=weights, reduction="sum")
        cb_loss /= torch.sum(labels_one_hot)
    return cb_loss
